...
const ChatBox = React.memo(
  ({ helpNeeded, setHelpNeeded }) => {
    ...
    const currentTime = useSelector(
      (state) => state.elearningState.timeClicked
    );
      ...
      if (helpNeeded) {
        setUserPrompt(
          "دقیقه" +
            ...
            "ویدیو آموزشی را بیشتر توضیح بده"
        );
      }
    }, [helpNeeded]);
    ...
    const handleSendMessage = async () => {
      ...
      try {
        ...
        const response = await sendPrompt(userPrompt, helpNeeded, currentTime);
